...
static void mbest_insert(struct MBEST *, uint16_t [], float);

struct MBEST *mbest_create() {
...
static void mbest_insert(struct MBEST *mbest, uint16_t index[], float error) {
...
void mbest_search(const float *cb, float vec[], struct MBEST *mbest, uint16_t index[]) {
    for (int j = 0; j < AMP_M; j++) {
        ...
        index[0] = j;
        mbest_insert(mbest, index, error);
    }
}
